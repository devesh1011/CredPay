...
import { SpinnerIcon, WarningIcon, WalletIcon } from "@phosphor-icons/react";
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      ...
      const isTestMode =
        params.get("test") === "true" || process.env.NODE_ENV === "development";
  ...
  // Authentication check with proper WalletIcon state handling
  useEffect(() => {
    if (!isClient) return;

    // If WalletIcon is actively connecting/reconnecting, wait
    if (isConnecting || isReconnecting) {
      return;
    }

    // Give WalletIcon time to auto-reconnect (500ms grace period)
    const timeoutId = setTimeout(() => {
      // Once WalletIcon state is settled, check authentication
  ...
  if (
    !testModeActive &&
    (!isClient || !authChecked || isConnecting || isReconnecting)
  ) {
    return (
      <>
        <Header />
            ...
            <div className="text-center space-y-4">
              ...
              <div>
                <p className="text-lg font-medium">
                  Checking WalletIcon connection...
                </p>
                <p className="text-sm text-muted-foreground mt-1">
                  Please wait while we verify your WalletIcon
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="max-w-md text-center space-y-6">
              ...
              <div className="space-y-2">
                <h1 className="text-2xl font-bold">
                  WalletIcon Connection Required
                </h1>
                <p className="text-muted-foreground">
                  Please connect your WalletIcon to access the dashboard and
              ...
              <div className="bg-orange-50 border border-orange-200 rounded-xl p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="text-sm text-orange-900 text-left">
                    ...
                    <p>
                      The dashboard contains personalized features like your
                      unique username, payment history, and API keys that are
                      tied to your WalletIcon address.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <p className="text-sm text-muted-foreground">
                  Click the WalletIcon button in the header to connect
                </p>
                <Link
                  href="/"
  ...
  // Verify address exists before rendering dashboard (use test WalletIcon if in test mode)
